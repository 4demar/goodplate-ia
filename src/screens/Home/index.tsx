...
export function Home() {
  ...
  async function handleSelectImage() {
    try {
      ...
      if (status !== ImagePicker.PermissionStatus.GRANTED) {
        return Alert.alert('É necessario ter permissao para acessar galeria de fotos!')
      }
      ...
      if (response.canceled) {
        ...
        const imgManipulate = await ImageManipulator.manipulateAsync(
        ...
        setSelectedImageUri(imgManipulate.uri)
        foodDetect(imgManipulate.base64)
  ...
  async function foodDetect(imageBase64: string | undefined) {
    const response = await api.post(`/v2/models/${process.env.EXPO_PUBLIC_API_MODEL}/versions/${process.env.EXPO_PUBLIC_API_MODEL_ID}/outputs`, {
      "user_app_id": {
        'user_id': process.env.EXPO_PUBLIC_API_USER_ID,
        'app_id': process.env.EXPO_PUBLIC_API_APP
      },
      'inputs': [
        {
          'data': {
    ...
    setMsg(isVegetable ? '' : 'Adicione Vegetais em seu prato!')
